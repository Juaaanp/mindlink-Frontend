        ...
        <div>
            ...
            <div className="pt-24 min-h-screen bg-[#0a0a0a] text-white p-6 space-y-10 font-poppins">
                ...
                <div className="flex justify-end">
                    <button className="bg-cyan-600 hover:bg-cyan-700 transition px-5 py-2 rounded-lg font-semibold"
                        ...
                        {createdRequests.map((req, index) => (
                            <div key={index} className="bg-[#1a1a1a] p-4 rounded-lg shadow-md border border-neutral-700">
                                <div className="flex justify-between mb-2">
                                    ...
                                    <span className={`text-xs px-2 py-1 rounded-full ${req.state === "OPEN" ? "bg-green-600" : "bg-gray-600"
                                        }`}>
                                        {req.state}
                                    </span>
                                </div>
                        ...
                        {respondedRequests.map((req, index) => (
                            <div key={index} className="bg-[#1a1a1a] p-4 rounded-lg shadow-md border border-neutral-700">
                                <div className="flex justify-between mb-2">
                                    ...
                                    <span className={`text-xs px-2 py-1 rounded-full ${req.state === "RESOLVED" ? "bg-blue-600" : "bg-gray-600"
                                        }`}>
                                        {req.state}
                                    </span>
                                </div>
            ...
            {showModal && (
                <AddHelpRequestModal onClose={() => setShowModal(false)} />
            )}
        </div>
    );
}